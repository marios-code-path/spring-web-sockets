...
Using `and` operator lets us join this session with a `session.receive()` flow which reacts to incoming messages. In this case, we dont have any need it, but it's good to illustrate this side of functionality for the curious.
