...
Crete an instance of https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/web/reactive/handler/SimpleUrlHandlerMapping.html[`SimpleUrlHandlerMapping`], and add `URL` to `WebSocketHandler` mappings.
The same mapping style exists for CORS, thus we'll just compose a simple default map for all URL's given; this is a simplistic program.
